...
'''
def convert_to_json(n = 4, nowrite = False, path = None):
    ...
    with open(path+"/resources/base_text.txt", 'r', encoding = 'utf-8') as f:
        ...
        # send mc to file
        if not nowrite:
            with open(path+"/resources/mc.json", "w") as mcjson:
                mcjson.write(str(n)+'\n')
...
'''
def load_mc(path = None):
    if path == None:
        path = ".."
    mc = {}
    dict_length = 0
    with open(path+'/resources/mc.json', 'r') as mcjson:
